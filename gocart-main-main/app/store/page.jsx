...
import { getOrdersByStoreId } from "@/lib/actions/order"
import { getRatingsByProductId } from "@/lib/actions/rating"
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            for (const product of products) {
                const productRatings = await getRatingsByProductId(product.id)
                ratings.push(...productRatings)
            }
                        ...
                        <div key={index} className="flex max-sm:flex-col gap-5 sm:items-center justify-between py-6 border-b border-slate-200 text-sm text-slate-600 max-w-4xl">
                            <div>
                                <div className="flex gap-3">
                                    {review.user?.image && <Image src={review.user.image} alt="" className="w-10 aspect-square rounded-full" width={100} height={100} />}
                                    <div>
